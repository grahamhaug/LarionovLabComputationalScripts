...
sbatchTemplate=/home1/05793/rdg758/scripts/g16_SBATCH_template.txt
...
function makeSBATCH {
		...
		alloOptions=("($fsKinetics) Kinetics-of-selectiv" "($fsMech) Mechanistic-Studies" "($fsComp) Computational-Analys" "($fsExcited) Excited-state-reactiv" "($fsSOCI) SOCI-Xmers")
		PS3=$alloQ
		COLUMNS=0
...
select numCase in "${jobOptions[@]}"; do
        case $REPLY in
                1) #single job
                        ...
                        makeSBATCH
                        envsubst '${WHICHQ} ${HOURS} ${MINS} ${NUMCORE} ${NUMTASK} ${ALLOCATION} ${RETURNDIR}' </home1/05793/rdg758/scripts/g16_SBATCH_template.txt> ./$jobName
                        sendPool
                        echo ""
                ...
                2) #select jobs by keyword
                        ...
                        for subJob in $trimmedJobNames; do
                                envsubst '${WHICHQ} ${HOURS} ${MINS} ${NUMCORE} ${NUMTASK} ${ALLOCATION} ${RETURNDIR}' </home1/05793/rdg758/scripts/g16_SBATCH_template.txt> ./$subJob
                                cp ${subJob} $jobPool
                        done
                ...
                3) #range of jobs
                        ...
                        for num in "${sortedRange[@]}"; do
                                jobName=${noNum}${num}
                                envsubst '${WHICHQ} ${HOURS} ${NUMCORE} ${NUMTASK} ${ALLOCATION} ${MINS} ${RETURNDIR}' </home1/05793/rdg758/scripts/g16_SBATCH_template.txt> ./$jobName
				sendPool
