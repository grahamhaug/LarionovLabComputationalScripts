...
xyzExistOptions=("Extract .xyz files from an IRC.log" "Create G16 inputs from existing .xyz files" "Project the IRC onto user-specified coordinate") 
PS3=$xyzExistQ
COLUMNS=0
select xyzCase in "${xyzExistOptions[@]}"; do 
	case $REPLY in
		1)	#extract from irc.log
			...
			#make a pasteable output file for the reaction coordinate
			#combines two columns of data into one output file
			#get the point number
			awk '$1 ~ /^ *Point/ {printf "%-5s\n", $2}' ${logName}-ircdata.txt > temp1
			#get the energy column
			awk '$1 ~ /^ *Point/ {printf "%s\n", $6}' ${logName}-ircdata.txt > temp2
			#get the IRC value at each point number
			awk '$1 ~ /^ *Point/ {printf "%+9s\n", $11}' ${logName}-ircdata.txt > temp3
			#header
			printf "Point#\tEnergy.Hartree\t\tPos.IRC\n" > ${logName}-IRcoord.txt
			#stick the columns together into tab-delimited output file
			paste temp1 temp2 temp3 >> ${logName}-IRcoord.txt
			#banish the temp files
		...
		2)	#Create G16 inputs from existing .xyz files
			echo -e "\n${GREEN}Creating G16 input files for existing .xyz files...${NC}"
			
			#echo "option is pending - check back later"
			break;;

		3)	#Project the IRC onto user-specified coordinate
			echo "here"
			#user can enter program here - need an "ircdata.txt" file for the following to work
			#need to find the existing one or make a new one
			#check for existing data file
			#or make a new one - need a .chk file for the IRC, then - and a flag to the user that this is required
			
			#user needs to specify the name of their file like in 1) 
			#check if the file exists, etc. 
		
			#Once that is in, the below should work		
			#the "POINT" starting line #s
			sGeoms=$(awk '/Point/ {print FNR}' test.txt | tr '\n\r' ' ')
			IFS=' ' read -r -a sGeomArray <<< "$sGeoms"
			sCount="${#sGeomArray[@]}"

			#the "----" ending line #s; omit the first instance
			eGeoms=$(awk '/----------/ {print FNR}' test.txt | tail -n +2 | tr '\n\r' ' ')
			IFS=' ' read -r -a eGeomArray <<< "$eGeoms"
			eCount="${#eGeomArray[@]}"
			...
			for (( i=1; i<$sCount; i++ ));  do
				startVar=${sGeomArray[i]}
				endVar=${eGeomArray[j]}
				capGeom=$(sed -n "${startVar}, ${endVar}p" test.txt)
				currentGeom=$(echo "$capGeom" | awk '$1 ~ /[0-9]/ {print}')
				geomArray+=("$currentGeom")
				(( j+=1 ))
			done
			...
			#how many geoms were captured
			geomsCount="${#geomArray[@]}"
			echo "$geomsCount extracted"

			#get two atom indices to define a bond stretch
			#get the first atom index from the user
			userIndex1=$(echo -e "${GREEN}Enter first atom index: ${NC}")
			read -ep "$userIndex1" index1
			#get the second atom index from the user
			userIndex2=$(echo -e "${GREEN}Enter second atom index: ${NC}")
			read -ep "$userIndex2" index2

			#stick them together for job labeling
			indexLabel="${index1}-${index2}"

			#generate the new column of data - can then concatenate this like I did above to produce a tab-delimited file
			for (( i=0; i<$geomsCount; i++ )) do
			#get the xyz coords cooresponding to the indicated atom indices
			#first atom
			index1Line=$(echo "${geomArray[i]}" | awk -v k=$index1 'NR==k {printf "%-2s %15s %15s %15s \n", $2, $3, $4, $5}') 
			ind1X=$(echo $index1Line | awk '{print $1}')
			ind1Y=$(echo $index1Line | awk '{print $2}')
			ind1Z=$(echo $index1Line | awk '{print $3}')
			#second atom
			index2Line=$(echo "${geomArray[i]}" | awk -v k=$index2 'NR==k {printf "%-2s %15s %15s %15s \n", $2, $3, $4, $5}')
			ind2X=$(echo $index2Line | awk '{print $1}')
			ind2Y=$(echo $index2Line | awk '{print $2}')
			ind2Z=$(echo $index2Line | awk '{print $3}')

			#xyz distance formula
			distform="sqrt(($ind2X - $ind1X)^2 + ($ind2Y - $ind1Y)^2 + ($ind2Z - $ind1Z)^2)"
	
			#use Bash's janky printf and BC combo to calculate 3D distance between the two atoms - it's rounded to 5 decimal pts via printf
			dist=$(printf "%.5f\n" $(bc -l <<< "$distform"))
				
			#append all the data to a txt for now - better will be a temp file to concatenate via paste
			printf "$dist\n" >> coord-atoms-"${indexLabel}".txt
			done
			break;;
