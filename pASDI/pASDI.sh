	...
	select initCase in "${initOptions[@]}"; do
		case $REPLY in
			1) #Pick from a list
				...
				dfaOptions=("M06-2X(D3)" "wB97X-D" "PW6B95(D3BJ)" "B3LYP(D3BJ)" "CAM-B3LYP(D3BJ)")
				PS3=$selectDFA
...
select xyzCase in "${xyzExistOptions[@]}"; do 
	case $REPLY in
		1)	#extract from irc.log
			...
			echo -e "\n${CYAN}Extracting full geometries from ${PURP}${logName}.log....${NC}"
...
select nextCase in "${whatNextOptions[@]}"; do
        case $REPLY in
			...
			for subjob in $jobsToMake; do
				...
				envsubst '${NUMTASK} ${MEM} ${DFA} ${DISPERSION} ${BASIS} ${SOLVATION} ${JOBTYPE} ${JOBNAME} ${CHECKLOC} ${CHARGE} ${MULT} ${GEOM}' </home1/05793/rdg758/scripts/G16_Input_Template.txt> ./$pasdiJob.com	
			done
